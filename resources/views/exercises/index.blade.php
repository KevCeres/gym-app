<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="font-semibold text-xl text-gray-900 leading-tight dark:text-gray-100">Ejercicios</h2>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Catálogo para atletas y entrenos</p>
        </div>
    </x-slot>
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/40 ring-1 ring-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:shadow-none dark:ring-slate-800">
                ...
                <div class="flex flex-col gap-4 border-b border-slate-100 p-5 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                    <div class="relative max-w-md flex-1" role="search">
                        <label for="exercise-search" class="sr-only">Buscar ejercicio</label>
                        ...
                        >
                        <button type="button" id="exercise-search-clear" class="absolute inset-y-0 end-0 hidden items-center pe-2 text-xs font-semibold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200" aria-hidden="true">Limpiar</button>
                    </div>
                    <a href="{{ route('exercises.create') }}" class="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500">
                ...
                <div id="exercise-no-results" class="hidden border-b border-amber-100 bg-amber-50/90 px-5 py-4 text-center text-sm font-medium text-amber-900 dark:border-amber-900/30 dark:bg-amber-950/30 dark:text-amber-100" role="status"></div>
                    ...
                    <table class="min-w-full divide-y divide-slate-100 text-sm dark:divide-slate-800">
                        <thead>
                            <tr class="bg-slate-50/90 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800/60 dark:text-slate-400">
                                <th scope="col" class="w-px px-4 py-3 sm:px-6">Vista</th>
                                <th scope="col" class="px-4 py-3 sm:px-6">Nombre</th>
                                <th scope="col" class="hidden px-4 py-3 sm:table-cell sm:px-6">Categoría</th>
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800/80">
                            @forelse ($exercises as $exercise)
                                @php
                                    $hay = mb_strtolower($exercise->name.' '.($exercise->category->name ?? ''));
                                @endphp
                                <tr data-admin-row data-search-haystack="{{ e($hay) }}" class="transition hover:bg-indigo-50/40 dark:hover:bg-slate-800/50">
                                    <td class="px-4 py-3 align-middle sm:px-6">
                                        <x-exercise-thumbnail :exercise="$exercise" class="ring-2 ring-white dark:ring-slate-800" />
                                    </td>
                                    <td class="px-4 py-3 sm:px-6">
                                        <p class="font-semibold text-slate-900 dark:text-white">{{ $exercise->name }}</p>
                                        <p class="mt-0.5 text-xs text-slate-500 sm:hidden dark:text-slate-400">{{ $exercise->category->name ?? '—' }}</p>
                                    </td>
                                    <td class="hidden px-4 py-3 sm:table-cell sm:px-6">
                                        <span class="inline-flex rounded-lg bg-indigo-50 px-2 py-0.5 text-xs font-semibold text-indigo-800 dark:bg-indigo-950/50 dark:text-indigo-200">{{ $exercise->category->name ?? '—' }}</span>
                                    </td>
                                    <td class="whitespace-nowrap px-4 py-3 text-end sm:px-6">
                                        <div class="inline-flex items-center justify-end gap-1">
                                            <a href="{{ route('exercises.edit', $exercise) }}" class="inline-flex items-center justify-center rounded-lg p-2 text-indigo-600 transition hover:bg-indigo-50 dark:text-indigo-400 dark:hover:bg-indigo-950/40" title="Editar">
                                                <span class="sr-only">Editar</span>
                                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>
                                            </a>
                                            <form action="{{ route('exercises.destroy', $exercise) }}" method="POST" class="inline" onsubmit="return confirm('¿Eliminar este ejercicio?');">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-slate-500 dark:text-slate-400">No hay ejercicios. Crea el primero.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <p class="border-t border-slate-100 px-5 py-3 text-center text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400 sm:px-6">
                    Mostrando <span id="exercise-count-label">{{ $exercises->count() }}</span> de {{ $exercises->count() }} ejercicios
                </p>
            </div>
        </div>
    </div>
    ...
    @if($exercises->isNotEmpty())
        <script>
            (function () {
                ...
                var clearBtn = document.getElementById('exercise-search-clear');
                var noResults = document.getElementById('exercise-no-results');
                var countLabel = document.getElementById('exercise-count-label');
                if (!input) return;
                ...
                function filter() {
                    ...
                    if (noResults) {
                        ...
                        if (q !== '' && visible === 0) noResults.textContent = 'Ningún ejercicio coincide con «' + q + '».';
                    }
